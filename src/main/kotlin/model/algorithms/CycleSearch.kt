...
class CycleSearch<D>(private val graph: UndirectedGraph<D>) {
    ...
    private fun findAnyCycle(vertexId: Int): HashMap<Int, Int>? { // возвращает null при отсутствии цикла
    ...
    fun findCycle(vertex: Vertex<D>): UndirectedGraph<D>? {
        ...
        if (graph.adjacency[vertex.id]!!.size < 2) {
            return null
        } else { // далее рассматриваем всевозможные случаи цикла посредством выбора пары соседей для возможной минимизации найднного цикла
